...
#define EMPTY 'n' 
#define STAY 'y' FILE *f_ro;
 FILE *f_doc;
 FILE *f_pat; FILE *f_ty;
 FILE *f_pwd;
 struct Login{ char pwd[15];
char usr[15]; }; 
struct Types{ int co;
 char na[10]; float pri; 
}; 
struct Room{ int no;
 struct Types type;
 char sta; 
...
int min; 
int sec; };
 struct Doctor{ char id[10]; 
char fname[15]; char lname[15];
 char sex; char blo[5]; 
struct date birth; char nat[20];
 char spec[20]; struct DateTime join;
 char tel[20]; char email[35]; 
char addr[40]; 
};
 struct Patient
{
char id[10];
 char fname[15];
 char lname[15]; char sex; char blo[5]; struct date birth; char prob[35]; char nat[20]; char tel[20]; char addr[40]; struct DateTime regi; int ro;
};
...
void alltype();
 int menu(int); 
int doctor(); 
int patient(); int dropmenu(char menu[7][20],int x1,int y1,int x2,int y2,int opt); void box(int x1,int y1,int x2,int y2,int c); void clrwin(); int program(); int about(); int room(); char *input(int,int,int); float inputf(int,int); /*end Function Declaration*/ void main(){ createfile(); _setcursortype(0); int opt=0; if(login(3)==1) goto start; exit(0); start: welc(); textbackground(14); clrscr(); gotoxy(22,1);textcolor(9);cprintf("==== Hospital Management System ===="); do{ switch(menu(opt)){ case 0:opt=program();break; case 1:opt=doctor();break; case 2:opt=patient();break; case 3:opt=room();break; case 4:opt=about();break; } }while(1); } int resetpwd(int opt){ struct Login log; int s,sh=0,h=0; char usr[20],pwd[20],ch; char code[8]; char head[2][30]={"SET USER","RESET USER"}; int t=0; do{ h=checkf_pwd(); if(opt==1){ clrwin(); }else{ textbackground(9); window(1,1,80,25); clrscr(); gotoxy(22,4);textcolor(9);cprintf("==== Hospital Management System ===="); box(2,2,79,24,15); } drawwindow(head[h],20,7,60,19,12);
 if(ch==13){ 
